...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('mutasiUpdate', $mutasi->id_mutasi) }}" method="post">
                @csrf
                @method('PUT')
                        ...
                        <select name="id_barang" class="form-control @error('id_barang') is-invalid @enderror">
                            ...
                            @foreach ($barang as $b)
                                <option value="{{ $b->id_barang }}"
                                    {{ old('id_barang', $mutasi->id_barang) == $b->id_barang ? 'selected' : '' }}>
                                    {{ $b->nama_barang }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row mb-3">
                        ...
                        <select name="dari_bidang" class="form-control @error('dari_bidang') is-invalid @enderror">
                            @foreach ($bidang as $bd)
                                <option value="{{ $bd->id_bidang }}"
                                    {{ old('dari_bidang', $mutasi->dari_bidang) == $bd->id_bidang ? 'selected' : '' }}>
                                    {{ $bd->nama_bidang }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-xl-6 mb-1">
                        ...
                        <select name="ke_bidang" class="form-control @error('ke_bidang') is-invalid @enderror">
                            ...
                            @foreach ($bidang as $bd)
                                <option value="{{ $bd->id_bidang }}"
                                    {{ old('ke_bidang', $mutasi->ke_bidang) == $bd->id_bidang ? 'selected' : '' }}>
                                    {{ $bd->nama_bidang }}
                                </option>
                            @endforeach
                        </select>
