        ...
        <div class="card-body">
            <form action="{{ route('barangStore') }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="kode_barang" class="form-control @error('kode_barang') is-invalid @enderror" value="{{ old('kode_barang') }}" autocomplete="off">
                    @error('kode_barang')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('id_jenis') is-invalid @enderror" name="id_jenis">
                        <option disabled {{ old('id_jenis') ? '' : 'selected' }}>-- PILIH JENIS --</option>
                        @foreach ($jenis as $item)
                            <option value="{{ $item->id }}" {{ old('id_jenis') == $item->id ? 'selected' : '' }}>
                                {{ $item->jenis_barang }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('id_merk') is-invalid @enderror" name="id_merk">
                        <option disabled {{ old('id_merk') ? '' : 'selected' }}>-- PILIH MERK --</option>
                        @foreach ($merk as $item)
                            <option value="{{ $item->id }}" {{ old('id_merk') == $item->id ? 'selected' : '' }}>
                                {{ $item->merk }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('id_tipe') is-invalid @enderror" name="id_tipe">
                        <option disabled {{ old('id_tipe') ? '' : 'selected' }}>-- PILIH TIPE --</option>
                        @foreach ($tipe as $item)
                            <option value="{{ $item->id }}" {{ old('id_tipe') == $item->id ? 'selected' : '' }}>
                                {{ $item->tipe }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="tahun_pembelian" class="form-control" value="{{ old('tahun_pembelian') }}">
                </div>
